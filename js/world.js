...
var ROOM_CORRIDOR = new Room ("Corridor", "You are in the corridor. Just a plain corridor, nothing special.",
	"To the south is the bathroom, to the west is the computer lab called â€œOrangeâ€, to the north is the lecture hall.");
...
var ITEM_NERD = new Item("nerd", "A nerd.", "Some nerd is programming on his Macbook. What a hipster. Better keep it down though, wouldnâ€™t want to disturb him.", "Are you out of your mind?!");
...
// ==== ORANGE ====
var ROOM_ORANGE = new Room ("Orange", "You are in the computer lab Orange. It seems like all of the computers have been moved, thatâ€™s weird.", 
			...
			if(!isMentioned("using", params)) {
				...
				if(isMentioned("toilet", params) || isMentioned("tap", params)) {
					...
					if(isMentioned("toilet", params)) {
						//Used toiled
						reply("You fill the bottle with toilet water because doing that makes perfect sense. Your empty bottle just transformed intoâ€¦ a water bottle. Amazing!");

					} else {
						reply("You decide to do the sane thing and fill the bottle with water from the tap instead of the toilet. Good for you. Just donâ€™t expect an award. Your empty bottle just transformed intoâ€¦ a water bottle. Shocking!");
					}
					return;
				} else {
					reply("Don't think so...");
...
ITEM_COMPUTER.addAction("use", function() {
	reply("You browse Facebook, seems like you have a notification. Oh, only an invite to play Candy Crush. What did you expect, you study computer science. You donâ€™t have a social life.");
});
...
ITEM_MICHAEL.addAction("talk", function(){
	if(PLAYER.hasItem("key")){
		...
		reply("Michael looks up at you, he tells you heâ€™s thirsty then keeps on reading. Maybe you could help him.");
	}
});
...
var ITEM_NUMBER = new Item("Number", "A number on the wall.", "On one of the walls there is something written: â€œCall this number for a good timeâ€ followed by a telephone number. Seems legit.", "You can't remove this from the wall");
